...
@echo put 'file://%~dp0CSV\%%~nj*.csv.gz' @%SOURCE%.CERTS_STAGE/%LOAD_DATE%/%%~nj OVERWRITE=true;>>BW_SNOWFLAKE_LOAD.sql
)

for /r %%k in (BCP\*.bcp) do (
@echo copy into %SOURCE%.%%~nk from @%SOURCE%.CERTS_STAGE/%LOAD_DATE%/%%~nk file_format=(format_name=PIPE_UTF16^);>>BW_SNOWFLAKE_LOAD.sql
)
