...
@echo put 'file://%~dp0CSV\%%~nj*.csv.gz' @%SOURCE%.FUNDATA_STAGE/%LOAD_DATE%/%%~nj OVERWRITE=true;>>BW_SNOWFLAKE_LOAD.sql
)

for /r %%k in (BCP\*.bcp) do (
@echo copy into %SOURCE%.%%~nk from @%SOURCE%.FUNDATA_STAGE/%LOAD_DATE%/%%~nk file_format=(format_name=PIPE_UTF16^);>>BW_SNOWFLAKE_LOAD.sql
)
